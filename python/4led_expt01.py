...
class LED_driver(object):
    ...
    def light_led(self,LEDnum,colour,duty):
        self.pwm.setDuty(channels[LEDnum][colour], duty)
    ...
    def light_with_RGB(self,LEDnum,the_RGB):
        the_levels = self.rgb(the_RGB)
        # import pdb;pdb.set_trace()
        for col,val in the_levels.items():
            self.light_led(LEDnum,col,val)
...
if __name__ == "__main__":
    ...
    if len(args)<2:
        for i in range(0,4):
            the_leds.light_with_RGB(i,'888888')
    
    elif len(args)<3:
            print("The LED number as well as the command is required:: XXX.py LEDnum command")
    else:
        LEDnum = int(args[1])
        command = args[2]
        #import pdb;pdb.set_trace()
        if command != "cycle":
            the_leds.light_with_RGB(LEDnum,command)
        else:
            the_leds.light_with_RGB(LEDnum,"FF0000")
            for b in range(1,cycsize):
                bval = float(b/cycsize)
                the_leds.light_led(LEDnum,"GREEN",bval)
                time.sleep(0.001)
